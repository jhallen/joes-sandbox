...
int corder(NODE * list)
{
	...
	case tX:
		return 1;
	case tEXP:
		return corder(list->v.lst.l) *
		    (eval(list->v.lst.r, zero, zero).r);
	case tMUL:
...
C eval(NODE * list, C x, C k)
{
	...
	case tCONST:
		...
		return vv;
	case tADD:
		return add(eval(list->v.lst.l, x, k),
			   eval(list->v.lst.r, x, k));
	case tSUB:
		return sub(eval(list->v.lst.l, x, k),
			   eval(list->v.lst.r, x, k));
	case tMUL:
		return mul(eval(list->v.lst.l, x, k),
			   eval(list->v.lst.r, x, k));
	case tDIV:
		return jdiv(eval(list->v.lst.l, x, k),
			   eval(list->v.lst.r, x, k));
	case tNEG:
...
C muller()
{
	...
	h = sub(x, ix);
	gamma = jdiv(h, sub(ix, iix));
	goto into;
      loop:
	g = sub(mul(add(one, mul(two, gamma)), sub(f, iff)),
		mul(mul(gamma, gamma), sub(iff, iif)));
	bot = jcsqrt(sub(mul(g, g),
			mul(four,
			    mul(f,
				mul(add(one, gamma),
				    mul(gamma,
...
int main(int argc, char *argv[])
{
	...
	int order;
      ...
      mmmm:
	printf
	    ("Enter function in complex variable X and real parameter K\n");
      mmm1:
	if (!gets(str))
		return 0;
	ss = str;
...
/*  printf("%f %f\n",have[z*3].i,x.i);*/
			...
			array[z] = x;
			array[z + 1] = zero;
			fg_drawthickline(FG_YELLOW, FG_MODE_SET, ~0,
					 FG_LINE_SOLID, lin, fg_displaybox,
					 2);
		}
		sprintf(str, "K=%f", k.r);
		if (flag)
			fg_puts(FG_BLACK, FG_MODE_SET, ~0, FG_ROT0, 0, 0,
				olds, fg_displaybox);
		fg_puts(FG_YELLOW, FG_MODE_SET, ~0, FG_ROT0, 0, 0, str,
			fg_displaybox);
		strcpy(olds, str);
		flag = 1;
		if (bioskey(1)) {
