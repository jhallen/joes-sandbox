...
int *text;
 ...
 else if(isletter(text[x-1])) left=1;
 else left=2;

 if(!text[x+1]) right=0;
 else if((text[x+1]&255)==' ' || (text[x+1]&255)=='.' || (text[x+1]&255)==','
         || (text[x+1]&255)==':' || (text[x+1]&255)==';' || (text[x+1]&255)=='?') right=0;
 else if(isletter(text[x+1])) right=1;
 else right=2;
...
int *text;
 {
 int x;
 for(x=1;text[x];++x) if((255&text[x])=='_' && okbeg(text,x)) return 1;
 return 0;
 }
...
int *text, *siz, *len;
 ...
 for(x=0;text[x];++x)
  if((255&text[x])=='_' && okbeg(text,x) && (attr || thereisokend(text+x)))
   attr^=ITAL;
  else
