...
#define oprintf1(s, a0) ( \
    sprintf(out, (s), (a0)), \
    (out += strlen(out)) \
  )

#define oprintf2(s, a0, a1) ( \
    sprintf(out, (s), (a0), (a1)), \
    (out += strlen(out)) \
  )

#define oprintf3(s, a0, a1, a2) ( \
    sprintf(out, (s), (a0), (a1), (a2)), \
    (out += strlen(out)) \
  )
...
struct saved {
	...
	void (*func) (void);	/* If an internal function is bound to the name */
...
void set(struct level *lvl, char *name, char *text, void (*func) (void),
	 int argc, char **args)
{
	struct saved *s;
	int h = hash(name);
	...
	s->nargs = argc;
	if (argc)
		s->args = (char **) malloc(sizeof(char *) * argc);
	for (x = 0; x != argc; ++x)
		s->args[x] = strdup(args[x]);
}

void push(int nargs, char **args)
{
...
void gentext(char *s)
{
	...
	for (x = 0, y = 0; s[x]; ++x, ++y) {
		...
		outbuf[outx + y] =
		    c + (fheight << 8) + (fwidth << 16) + (fontsel << 24);
	}
	outx += y;
}
...
int width(int *outbuf, int len)
{
	...
	for (x = 0; x != len; ++x) {
		...
		int c = (255 & outbuf[x]);
		if (fonts[f].width)
			w += wth * (fonts[f].width + fonts[f].sep);
		else if (f == 8)
			w += wth;
		else
			w += (wth * (varfont[c] + 284)) / 8782;
	}
	return w;
}

int maxheight(int *outbuf, int len)
{
	int x;
	int b = 0;
	for (x = 0; x != len; ++x) {
		...
		int bas;
		if (fonts[f].width)
			bas = (fonts[f].height + fonts[f].vsep) * hgt;
		else
			bas = hgt + hgt / 6;
		if (bas > b)
			b = bas;
...
int maxabove(int *outbuf, int len)
{
	int x;
	int b = 0;
	for (x = 0; x != len; ++x) {
...
int maxbelow(char *outbuf, int len)
{
	int x;
	int b = 0;
	for (x = 0; x != len; ++x) {
		...
		int bas;
		if (fonts[f].width)
			bas = (fonts[f].height - fonts[f].base) * hgt;
		else
			bas = hgt / 4;
		if (bas > b)
			b = bas;
...
void tgen(int x, int base, int from, int to)
{
	int z;
	x += loffset;
	for (z = from; z != to;) {
		...
		if (ee) {
			...
			if (oy != ny)
				oprintf2("^FT%d,%d", x, ny + base);
			else
				oprintf1("^FT%d,", x);
			if (pf != f)
				if (fon == 8)
					oprintf2("^GSN,%d,%d^FD", h, w);
				else
					oprintf3("^CF%c,%d,%d^FD",
						 fonts[fon].id, h, w);
			else
...
void save()
{
	...
	if (*in == '[') {
		...
		if (*in != ']') {
			...
			*bp = 0;
			++in;
			set(root, get("name")->text, strdup(buf), 0,
...
void upc()
{
	...
	ox = -1;
	oy = -1;
	oprintf3("^FO%d,%d^BUN,100^FD%s^FS",
		 lwidth / 2 - 190 / 2 + loffset, y, get("code")->text);
	y += 100;
	mode = 2;
}
...
void vupc()
{
	...
	ox = -1;
	oy = -1;
	oprintf3("^FO%d,%d^BUR,100^FD%s^FS",
		 lwidth / 2 - 100 / 2 + loffset, y + 14,
		 get("code")->text);
	y += 218;
	mode = 2;
}
...
void init()
{
	...
	for (x = 0; x != sizeof(initab) / sizeof(struct init); ++x)
		set(top, initab[x].name, 0, initab[x].func,
		    initab[x].nargs, initab[x].args);
}
