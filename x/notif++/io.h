...
struct Output
  ...
  char *buf;			/* Data */
  int len;			/* Length */
  Fn_0<void> *write;		/* Function to call after data has been sent */
  };
...
struct Port
  ...
  Fn_2<void,char *,int> *read;	/* Callback to execute for read data */
  
  char *buf;			/* Input buffer */
  int siz;				/* Buffer siz */
  int nnew;				/* Write point */
  int old;				/* Read point */
  ...
  void doread();
  void dowrite();
  void dofinalwrite(char *data,int len,Fn_0<void> *fn);
  
  void stportread(Fn_2<void,char *,int> *new_read,int len);
  /*
  ...
  inline int portlen() { return taken; }
   ...
   */
  int write(char *buf,int len,Fn_0<void> *fn);
   ...
   */
  void finalwrite(char *buf,int len,Fn_0<void> *fn);
