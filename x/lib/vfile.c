...
mutex_t vfile_mutex[1]={MUTEX_INITIALIZER};
...
/* Write all dirty pages of a specific file in order of their position
   within the file */

static void dovflshf(vfile)
VFILE *vfile;
 {
 VPAGE *vp;
 VPAGE *vlowest;
 ...
 /* Find dirty page at lowest file address */
 for(x=0;x!=HTSIZE;x++)
  for(vp=htab[x];vp;vp=vp->next)
   if(vp->addr<addr && vp->vfile==vfile && vp->dirty /* && !vp->count */)
    addr=vp->addr, vlowest=vp;

 /* Check if we found a page to flush */
 if(vlowest)
  { /* We found a page to write */
   printf("Writing page %d\n",vlowest->addr);
#ifdef HAVEMMAP
#ifdef BROKENMMAP
  /* For broken mmap call we have to write all dirty pages */
  if(1)
#else
  /* For working mmap call we only have to write new pages */
  if(vlowest->addr>=vfile->size)
#endif
#else
  if(1)
#endif
   ...
   if(vfile->fd== -1) vfile->fd=open(vfile->name,O_RDWR);
   lseek(vfile->fd,addr,0);
   if(addr+PGSIZE>dovsize(vfile))
    { /* Partial page write */
    jwrite(vfile->fd,vlowest->data,(int)(dovsize(vfile)-addr));
    vfile->size=dovsize(vfile);
    }
   else
    { /* Full page write */
...
#ifdef HAVEMMAP
    /* Now that page is really part of the file, it can be mapped */
#ifdef BROKENMMAP
    vlowest->data=mmap(vlowest->data,PGSIZE,PROT_READ|PROT_WRITE,MAP_FILE|MAP_PRIVATE|MAP_FIXED,vfile->fd,addr);
#else
    vlowest->data=mmap(vlowest->data,PGSIZE,PROT_READ|PROT_WRITE,MAP_FILE|MAP_SHARED|MAP_FIXED,vfile->fd,addr);
#endif
#endif
    }
   }
...
VFILE *vfile;
 {
 mutex_lock(vfile_mutex);
 dovflshf(vfile);
 mutex_unlock(vfile_mutex);
 }

/* Write all dirty pages to file */

/* We flush all pages here: shouldn't we just flush a few at a time? */

static void dovflsh()
 {
 VFILE *vfile;
 for(vfile=vfiles.next;vfile!=(VFILE *)&vfiles;vfile=vfile->next)
  dovflshf(vfile);
 }
...
/* Allocate pages on specific memory alignment */
...
/* Get physical address of file offset.  Lock associated page into memory */
...
long addr;
 ...
 ofst=(addr&(PGSIZE-1));	/* Remember offset into page */
 addr-=ofst;			/* addr is now page address */

 mutex_lock(vfile_mutex);

 /* Lookup page address in hash table */
 for(vp=htab[((addr>>LPGSIZE)+(int)vfile)&(HTSIZE-1)];vp;vp=vp->next)
  if(vp->vfile==vfile && vp->addr==addr)
   /* We found the page in the hash table!  Inc reference count and return
    * address of data */
   {
   ++vp->count;
   mutex_unlock(vfile_mutex);
 ...
 /* Page is not in memory, so we have to map or read it in */
 ...
 if(curvalloc+PGSIZE<=maxvalloc)
  { /* Allocate a bunch of pages at once if we haven't reached the limit */
  vp=(VPAGE *)malloc(sizeof(VPAGE)*INC); /* Allocate some headers */
  if(vp)
   {
   vp->data=(char *)mema(PGSIZE,PGSIZE*INC); /* Allocated aligned pages */
   if(vp->data)
    {
    int q;
    curvalloc+=PGSIZE*INC;	/* Keep total alloced */
    /* Store header addresses in physical address to header array */
    if(!vheaders)
 ...
 /* This should really be an LRU algorithm... */

 /* Can't malloc a page, have to free up an existing one */
 /* Search through hash table for an unreferenced clean page */
 ...
 gotit:
 /* We have a free page to use */
 /* Record header data and put page in hash table */
 vp->addr=addr;
 vp->vfile=vfile;
 vp->dirty=0;
 vp->count=1;
 ...
 /* Read or map in page */
 if(addr<vfile->size)
  { /* Page exists in the file, so read it in */
  if(vfile->fd==-1) vfile->fd=open(vfile->name,O_RDWR);
  if(addr+PGSIZE>vfile->size)
   { /* Partial read for short file */
#ifdef HAVEMMAP
   /* Map anonymous memory into page (file can't be expanded with mmap) */
   vp->data=mmap(vp->data,PGSIZE,PROT_READ|PROT_WRITE,MAP_PRIVATE|MAP_ANON|MAP_FIXED,-1,0);
#endif
   lseek(vfile->fd,addr,0);
...
#ifdef HAVEMMAP
#ifdef BROKENMMAP
   vp->data=mmap(vp->data,PGSIZE,PROT_READ|PROT_WRITE,MAP_FILE|MAP_PRIVATE|MAP_FIXED,vfile->fd,addr);
#else
   vp->data=mmap(vp->data,PGSIZE,PROT_READ|PROT_WRITE,MAP_FILE|MAP_SHARED|MAP_FIXED,vfile->fd,addr);
#endif
#else
   /* Full read */
   lseek(vfile->fd,addr,0);
  ...
  vp->data=mmap(vp->data,PGSIZE,PROT_READ|PROT_WRITE,MAP_PRIVATE|MAP_ANON|MAP_FIXED,-1,0);
#else
  /* New page which is past end of file: just clear the page */
...
static long dovsize(vfile)
VFILE *vfile;
 {
 long size;
 if(vfile->left<vfile->lv)
  size=vfile->alloc+vfile->lv-vfile->left;
 else
  size=vfile->alloc;
 return size;
 }
...
VFILE *vfile;
 {
 if(vtell(vfile)==dovsize(vfile)) return MAXINT;
...
VFILE *vfile;
 {
 short w;
 if(vtell(vfile)+2>dovsize(vfile)) return -1;
 w=vgetc(vfile);
...
VFILE *vfile;
 {
 long w;
 if(vtell(vfile)+4>dovsize(vfile)) return -1;
 w=vgetc(vfile);
...
char c;
 {
 if(addr+1>dovsize(vfile)) valloc(vfile,addr+1-dovsize(vfile));
 if(vfile->vpage1) vunlock(vfile->vpage1);
 vfile->vpage1=vlock(vfile,vfile->addr=(addr&~(long)(PGSIZE-1)));
...
long addr;
 {
 short c;
 if(addr+2>dovsize(vfile)) return -1;
 c=rc(vfile,addr);
...
short c;
 {
 if(addr+2>dovsize(vfile)) valloc(vfile,addr+2-dovsize(vfile));
 wc(vfile,addr,c);
 wc(vfile,addr+1,c>>8);
 return c;
 }
...
long addr;
 {
 long c;
 if(addr+4>dovsize(vfile)) return -1;
 c=rc(vfile,addr);
...
long c;
 {
 if(addr+4>dovsize(vfile)) valloc(vfile,addr+4-dovsize(vfile));
 wc(vfile,addr,c);
 wc(vfile,addr+1,c>>8);
...
int size;
 ...
 char *src;
 int x;
 if(addr+size>dovsize(v)) valloc(v,addr+size-dovsize(v));
 while(size)
  {
  src=vlock(v,addr);
  x=PGSIZE-(addr&(PGSIZE-1));
