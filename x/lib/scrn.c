...
int c;
 ...
 if((t->attrib&FG_MASK)!=(c&FG_MASK))
  if(t->Sf) texec(t->cap,t->Sf,1,7-(((c&FG_MASK)>>FG_SHIFT)));
 if((t->attrib&BG_MASK)!=(c&BG_MASK))
  if(t->Sb) texec(t->cap,t->Sb,1,((c&BG_MASK)>>BG_SHIFT));
 t->attrib=c;
 }

/* Set scrolling region */

static void setregn(t,top,bot)
SCRN *t;
int top,bot;
 {
 if(!t->cs)
...
SCRN *t;
 ...
 if((ss-s>3 || s[w]!=' ') && t->ce)
  ...
  while(s!=ss)
   {
   *s=' '; t->cap->out(t->cap->outptr,' '); ++t->x; ++s;
   }
  }
 }
...
SCRN *t;
 ...
 ch=c;
 if(t->x!=x || t->y!=y) cpos(t,x,y);
 if(c-ch!=t->attrib) attr(t,c-ch);
 if(t->haz && ch=='~') ch='\\';
 t->cap->out(t->cap->outptr,ch);
 t->scrn[t->co*y+x]=c;
 ++t->x;
 }
...
CAP *cap;
 ...
 if(!getflag(t->cap,"in") && t->cap->baud<38400 )
  {
 ...
 if(t->cap->baud>=38400) t->scroll=0, t->insdel=0;
...
int x,y,*s,n;
 ...
 mcpy(t->scrn+x+t->co*y+n,t->scrn+x+t->co*y,(t->co-(x+n))*sizeof(int));
...
int x,y,n;
 ...
 mcpy(t->scrn+t->co*y+x,t->scrn+t->co*y+x+n,(t->co-(x+n))*sizeof(int));
...
int top,bot,amnt;
 ...
 mcpy(t->scrn+top*t->co,t->scrn+(top+amnt)*t->co,
...
int top,bot,amnt;
 ...
 mcpy(t->scrn+(top+amnt)*t->co,t->scrn+top*t->co,
       (bot-top-amnt)*t->co*sizeof(int));
 if(!top && t->da)
  {
...
SCRN *t;
 {
 msetI(t->scrn,' ',t->co*t->cap->skiptop);
 msetI(t->scrn+t->cap->skiptop*t->co,-1,(t->li-t->cap->skiptop)*t->co);
 msetI(t->sary,0,t->li);
 msetI(t->updtab+t->cap->skiptop,-1,t->li-t->cap->skiptop);
 t->x= -1;
 t->y= -1;
 t->top=t->li;
 t->bot=0;
