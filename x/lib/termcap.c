...
CAP *getcap(name,baud,out,outptr,dopadding)
char *name;
unsigned baud;
void (*out)();
void *outptr;
 ...
 if(tgetent(cap->tbuf,name)==1)
  return setcap(cap,baud,out,outptr,dopadding);
 else
  {
  free(cap->abuf);
  cap->abuf=0;
...
CAP *setcap(cap,baud,out,outptr,dopadding)
CAP *cap;
unsigned baud;
void (*out)();
void *outptr;
...
int l,a0,a1,a2,a3;
 ...
 while(c= *s++)
  if(c=='%' && *s)
   ...
   case 'b': --a; break;
   case 'a': c=escape(&s);
             if(escape(&s)=='p') x=a[escape(&s)-0100];
             else x=escape(&s);
             switch(c)
              {
              case '+': a[0]+=x; break;
...
int l,a0,a1,a2,a3;
 ...
 int total=0;
 void (*out)()=cap->out;
 void *outptr=cap->outptr;
 if(!s) return 10000;
 cap->out=totalup; cap->outptr= &total;
 texec(cap,s,l,a0,a1,a2,a3);
 cap->out=out; cap->outptr=outptr;
 return total;
 }
...
int a0,a1,a2,a3;
 ...
 int div=cap->div;
 if(!s) return 0;
 cap->out=cpl; cap->outptr= &ssp; cap->div=10000;
 ssp=vsmk(10);
 texec(cap,s,0,a0,a1,a2,a3);
 cap->out=out; cap->outptr=outptr; cap->div=div;
 return ssp;
 }
