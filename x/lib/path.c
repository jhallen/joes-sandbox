...
char *pwd()
 ...
 if(s) return vsncpy(NULL,0,sz(s));
 else return 0;
 }
...
struct direct
 ...
 char d_name[16];
 int dirstate;
 struct ffblk ffblk[1];
 };

struct direct *opendir(path)
char *path;
 {
 struct direct *d=malloc(sizeof(struct direct));
 d->dirstate=0;
 return d;
 }
...
struct direct *d;
 ...
 if(d->dirstate)
  {
  if(findnext(d->ffblk)) return 0;
  }
 else
  {
  if(findfirst("*.*",d->ffblk,FA_DIREC)) return 0;
  d->dirstate=1;
  }
 zcpy(d->d_name,d->ffblk->ff_name);
 for(x=0;d->d_name[x];++x) d->d_name[x]=todn(d->d_name[x]);
 return d;
 }
