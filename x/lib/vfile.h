...
/* Define if OS has mmap system call */
#define HAVEMMAP 1

/* Define if mmap system call is broken such that it can't map file pages
 * as shared (as in Linux 1.2.13). */
/* #define BROKENMMAP 1 */
...
struct vfile
 {
 QITEM(VFILE);			/* Doubly linked list of vfiles */
...
/* Return file offset from memory offset */

#define vofst(p) (vheader(p)->addr)
...
#define vchanged(vpage) ( mutex_lock(vfile_mutex), \
                          vheader(vpage)->dirty=1, \
...
#define vunlock(vpage)  ( mutex_lock(vfile_mutex), \
                          --vheader(vpage)->count, \
                          mutex_unlock(vfile_mutex) )
...
#define vupcount(vpage) ( mutex_lock(vfile_mutex), \
                          ++vheader(vpage)->count, \
                          mutex_unlock(vfile_mutex) )
