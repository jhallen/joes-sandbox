		...
		if (typ(q) == tLST) {
			if (q->d == yMUL) {
				...
				if (qq && typ(qq) == tNUM)
					*t = ((NUM *) qq)->n, ((NUM *) qq)->n =
					    1.0;
				else if (qq && typ(qq) == tLST && qq->d == yMUL
...
int rmv(LST *n, LST *q)
{
	if (n && typ(n) == tLST && (n->d == yADD || n->d == ySUB)) {
		if (match(n->r->d, q)) {
			discard(n->r->d);
			n->r->d = newn(1.0);
		} else
...
*/
 ...
 in:
	discard(n);
	return ncons(3, yMUL,
		     ncons(3, ySUB, dup(lookup("%0")->bind),
			   ncons(3, yDIV,
				 ncons(3, yADD,
				       ncons(2, yNEG, dup(lookup("%2")->bind)),
				       ncons(2, ySQRT,
					     ncons(3, ySUB,
						   ncons(3, yEXP,
							 dup(lookup("%2")->
