...
/* Skip whitespace */
	...
	if ((*str >= '0' && *str <= '9') || *str == '.') {
		...
		int x;
		node->what = wVAR;
		for (x = 0; (str[x] >= 'A' && str[x] <= 'Z') || (str[x] >= 'a' && str[x] <= 'z') || (str[x] >= '0' && str[x] <= '9') || str[x] == '_'; ++x)
			node->name[x] = str[x];
		node->name[x] = 0;
		str += x;
		...
		if (*str == '(') {
			...
			if (*str == ')')
				...
				printf("Missing right parenthasis\n");
		}
	} else if (*str == '(') {
		/* Found a parenthasis */
		free(node);
...
double ev(struct node *node)
{
	...
	case wNUM:
		return node->value;
	case wEXP:
		return pow(ev(node->left), ev(node->right));
	case wMUL:
	...
	case wFUNC:		/* Function call lookup */
		if (!strcmp(node->name, "sin"))
			return sin(ev(node->right));
		else if (!strcmp(node->name, "cos"))
			return cos(ev(node->right));
		else if (!strcmp(node->name, "tan"))
			return tan(ev(node->right));
		else {
...
void unparselisp(struct node *node)
{
	switch (node->what) {
	case wNUM:
		printf("%g", node->value);
...
void unparseforth(struct node *node)
{
	switch (node->what) {
	case wNUM:
		printf("%g", node->value);
		break;
	case wEXP:
		unparseforth(node->left);
		printf(" ");
		unparseforth(node->right);
		printf(" ^");
		break;
	case wMUL:
		unparseforth(node->left);
...
void unparsefortran(struct node *node, int prec)
{
	int opprec;
	...
	switch (node->what) {
	case wEXP:
		unparsefortran(node->left, 40);
		printf("^");
		unparsefortran(node->right, 40);
		break;
	case wMUL:
		unparsefortran(node->left, 20);
		printf("*");
